...
public class LogMessageTableModel implements TableModel {
    ...
    /**
     * Replace current message to the list with given message 
     * 
     * @param msg Message list to replace
     */
    public void replaceLastMessage(LogMessage msg) {
    ...
    public Object getValueAt(int row, int col) {
        ...
        if (canmsg == null) {
            ...
            switch (col) {
                case 0:
					return String.format("%6d (%d)", messages.get(row).getAbsTimestamp(), messages.get(row).getRelTimestamp());
                case 1:
                    return icons[messages.get(row).getType().ordinal()];
